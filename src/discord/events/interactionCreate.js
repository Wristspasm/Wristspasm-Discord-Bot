	...
	async execute(interaction) {
        ...
        if (interaction.isSelectMenu()) {
            ...
            if (interaction.values[0].includes('command.guildexpcheck.selectMenu')) {
                ...
                await interaction.reply({ files: [ "data/filteredExp.txt" ], content: `**Filtered Weekly Guild Experience** (${value})`, components: [button]  })
            }
        }   
        if (interaction.isButton()) {
            try {
                ...
                if (interaction.customId.includes('guild.apply_button')) {
                    ...
                    if ((await interaction.guild.members.fetch(interaction.user)).roles.cache.has(config.discord.linkedRole)) {
                        let meetRequirements = false
                            ...
                            hypixel.getPlayer(uuid).then(async player => {
                                ...
                                if (meetRequirements) {
                                    ...
                                    const noRequirementsEmbed = new EmbedBuilder()
                                        .setColor(15548997)
                                        .setAuthor({ name: 'An Error has occurred'})
                ...
                // ? /gexpcheck command
                if (interaction.customId.includes('guild.guildexpcheck.button.kick_inactive')) {
                    if ((await interaction.guild.members.fetch(interaction.user)).roles.cache.has(config.discord.commandRole)) {
                        ...
                        for (const member of guildMembers) {
                            ...
                            await delay(690)
                            i++;
                            member.split(' » ')[0] == '' ? '' : kicked += i%5 == 0 ? `${member.split(' » ')[0]}\n` : `${member.split(' » ')[0]} | `
                            if (toFixed(i/guildMembers.length*100, 0).endsWith('0') || toFixed(i/guildMembers.length*100, 0).endsWith('5')) await interaction.editReply({content: `**Kick Purge Started**\n\`Progress:\` ${toFixed(i/guildMembers.length*100, 2)}%`})
                        }
                        kicked = kicked.slice(0, -3)
                        await interaction.followUp({content: `**Kicked Members**\n\`${kicked}\``})
                    } else {

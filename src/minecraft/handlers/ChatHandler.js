  ...
  async onMessage(event) {
    ...
    if (this.isRequestMessage(message)) {
      ...
      if (config.guildRequirement.enabled) {
        ...
        const weight = (await getWeight(profile.profile, profile.uuid)).weight
          .senither.total;
        ...
        if (weight > config.guildRequirement.requirements.senitherWeight)
          meetRequirements = true;

        if (bwLevel > config.guildRequirement.requirements.bedwarsStars)
          meetRequirements = true;
        if (
          bwLevel > config.guildRequirement.requirements.bedwarsStarsWithFKDR &&
          bwFKDR > config.guildRequirement.requirements.bedwarsFKDR
        )
          meetRequirements = true;

        if (swLevel > config.guildRequirement.requirements.skywarsStars)
          meetRequirements = true;
        if (
          swLevel > config.guildRequirement.requirements.skywarsStarsWithKDR &&
          swKDR > config.guildRequirement.requirements.skywarsStarsWithKDR
        )
          meetRequirements = true;

        if (duelsWins > config.guildRequirement.requirements.duelsWins)
          meetRequirements = true;
        if (
          duelsWins > config.guildRequirement.requirements.duelsWinsWithWLR &&
          dWLR > config.guildRequirement.requirements.duelsWinsWithWLR
        )
          meetRequirements = true;

        bot.chat(
          `/oc ${username} ${
            meetRequirements ? "Does" : "Doesn't"
          } meet Requirements. [BW] [${player.stats.bedwars.level}✫] FKDR:${
            player.stats.bedwars.finalKDRatio
          } | [SW] [${player.stats.skywars.level}✫] KDR:${
            player.stats.skywars.KDRatio
          } | [Duels] Wins: ${player.stats.duels.wins} WLR: ${
          ...
          const statsEmbed = new EmbedBuilder()
            ...
            .addFields(
              ...
              },
              {
                ...
                value: `${addCommas(toFixed(weight, 2))}`,
                inline: true,
              }
            )
    ...
    if (this.isJoinMessage(message)) {
      ...
      await delay(1000);
      bot.chat(
        "/gc Welcome to the guild! Make sure to join our discord /g discord! To view my commands run !help, Have a nice day :D | By DuckySoLucky#5181"
      );
      return [
        this.minecraft.broadcastHeadedEmbed({

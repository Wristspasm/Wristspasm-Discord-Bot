...
class SkyblockCommand extends minecraftCommand {
  ...
  async onCommand(username, message) {
    try {
      ...
      this.send(
        `/gc ${username}'s Senither Weight » ${senitherWeight} | Lily Weight » ${lilyWeight} | Skill Average » ${skillAverage} | Slayer » ${slayerXp} | Catacombs » ${catacombsLevel} | Class Average » ${classAverage} | Networth » ${networthValue} | Accessories » ${talismanCount} | Recombobulated » ${recombobulatedCount} | Enriched » ${enrichmentCount}`
      );
    } catch (error) {
      console.log(error);
      this.send(

...
class renderCommand extends MinecraftCommand {
  ...
  async onCommand(username, message) {
    ...
    try {
      const searchedPlayer = await getPlayer(username).catch((err) => {
        this.send(`/gc Error: ${err}`);
      });
      const playerProfile = searchedPlayer.memberData;
      const inventory = playerProfile?.inv_contents?.data;
      if (!inventory) {
        this.send(`/gc This player has an Inventory API off.`);
      }
      const inventoryData = (await decodeData(Buffer.from(inventory, "base64")))
        .i;
      ...
      const renderedItem = await renderLore(
        ...
        selectedItem?.tag?.display?.Lore
      );
      const upload = await imgurClient.upload({
        image: renderedItem,
        type: "stream",
      });
      if (searchedPlayer.profileData.game_mode == "ironman")
        username = `♲ ${username}`;
      this.send(
        `/gc ${username}'s item at slot ${itemNumber} » ${
          upload.data.link ?? "Something went Wrong.."
